...
// --- show-jks and from-jks CLI tests ---

type jksCLIFakeKeytool struct{}

func (jksCLIFakeKeytool) Run(_ context.Context, args ...string) ([]byte, []byte, error) {
	if len(args) == 0 {
		return nil, nil, nil
	}
	switch args[0] {
	case "-list":
		return []byte("Alias name: myalias\nEntry type: trustedCertEntry\nOwner: CN=test\n"), nil, nil
	case "-exportcert":
		for i, a := range args {
			if a == "-file" && i+1 < len(args) {
				_ = os.WriteFile(args[i+1], []byte{0x30, 0x82, 0x01, 0x00}, 0o644)
			}
		}
		return nil, nil, nil
	}
...
func TestShowJKS_JSON(t *testing.T) {
	oldIsTTY := isTerminalFn
	t.Cleanup(func() { isTerminalFn = oldIsTTY })
	isTerminalFn = func(_ *os.File) bool { return false }

	dir := t.TempDir()
	jksPath := filepath.Join(dir, "test.jks")
	_ = os.WriteFile(jksPath, []byte("dummy"), 0o644)

	engine := cert.NewDefaultEngine()
	engine.SetKeytool(jksCLIFakeKeytool{})

	cmd := NewRootCmd(engine, nil, BuildInfo{Version: "test"})
	var out bytes.Buffer
	cmd.SetOut(&out)
	cmd.SetErr(&bytes.Buffer{})
	cmd.SetArgs([]string{"show-jks", jksPath, "--json"})

	if err := cmd.Execute(); err != nil {
		t.Fatalf("expected no error, got %v", err)
	}
	var result cert.JKSSummary
	if err := json.Unmarshal(out.Bytes(), &result); err != nil {
		t.Fatalf("expected valid JSON, got %q err=%v", out.String(), err)
	}
...
func TestShowJKS_HumanOutput(t *testing.T) {
	oldIsTTY := isTerminalFn
	t.Cleanup(func() { isTerminalFn = oldIsTTY })
	isTerminalFn = func(_ *os.File) bool { return false }
	...
	cmd.SetOut(&out)
	cmd.SetErr(&bytes.Buffer{})
	cmd.SetArgs([]string{"--plain", "show-jks", jksPath})
...
func TestShowJKS_MissingFile(t *testing.T) {
	oldIsTTY := isTerminalFn
	t.Cleanup(func() { isTerminalFn = oldIsTTY })
	isTerminalFn = func(_ *os.File) bool { return false }
	...
	cmd.SetOut(&bytes.Buffer{})
	cmd.SetErr(&bytes.Buffer{})
	cmd.SetArgs([]string{"show-jks", "/nonexistent.jks"})
...
func TestShowJKS_NoArgs(t *testing.T) {
	oldIsTTY := isTerminalFn
	t.Cleanup(func() { isTerminalFn = oldIsTTY })
	isTerminalFn = func(_ *os.File) bool { return false }
...
func TestShowJKS_PasswordStdin(t *testing.T) {
	oldIsTTY := isTerminalFn
	t.Cleanup(func() { isTerminalFn = oldIsTTY })
	isTerminalFn = func(_ *os.File) bool { return false }
	...
	cmd.SetOut(&out)
	cmd.SetErr(&bytes.Buffer{})
	cmd.SetIn(strings.NewReader("changeit\n"))
	cmd.SetArgs([]string{"show-jks", jksPath, "--password-stdin", "--json"})
...
func TestFromJKS_CLI_JSON(t *testing.T) {
	oldIsTTY := isTerminalFn
	t.Cleanup(func() { isTerminalFn = oldIsTTY })
	isTerminalFn = func(_ *os.File) bool { return false }
	...
	cmd.SetErr(&bytes.Buffer{})
	outDir := filepath.Join(dir, "extracted")
	cmd.SetArgs([]string{"from-jks", jksPath, outDir, "--json"})

	if err := cmd.Execute(); err != nil {
		t.Fatalf("expected no error, got %v", err)
	}
	var result cert.FromJKSResult
	if err := json.Unmarshal(out.Bytes(), &result); err != nil {
		t.Fatalf("expected valid JSON, got %q err=%v", out.String(), err)
	}
	if len(result.CertFiles) == 0 {
...
func TestFromJKS_CLI_HumanOutput(t *testing.T) {
	oldIsTTY := isTerminalFn
	t.Cleanup(func() { isTerminalFn = oldIsTTY })
	isTerminalFn = func(_ *os.File) bool { return false }
	...
	cmd.SetErr(&bytes.Buffer{})
	outDir := filepath.Join(dir, "extracted")
	cmd.SetArgs([]string{"--plain", "from-jks", jksPath, outDir})
	...
	if !strings.Contains(got, "Certificate:") {
		t.Errorf("expected certificate output, got %q", got)
	}
}

func TestFromJKS_CLI_WithAlias(t *testing.T) {
	oldIsTTY := isTerminalFn
	t.Cleanup(func() { isTerminalFn = oldIsTTY })
	isTerminalFn = func(_ *os.File) bool { return false }
	...
	cmd.SetErr(&bytes.Buffer{})
	outDir := filepath.Join(dir, "extracted")
	cmd.SetArgs([]string{"from-jks", jksPath, outDir, "--alias", "myalias", "--json"})
...
func TestFromJKS_CLI_MissingFile(t *testing.T) {
	oldIsTTY := isTerminalFn
	t.Cleanup(func() { isTerminalFn = oldIsTTY })
	isTerminalFn = func(_ *os.File) bool { return false }
	...
	cmd.SetOut(&bytes.Buffer{})
	cmd.SetErr(&bytes.Buffer{})
	cmd.SetArgs([]string{"from-jks", "/nonexistent.jks", "/tmp/out"})
...
func TestFromJKS_CLI_NoArgs(t *testing.T) {
	oldIsTTY := isTerminalFn
	t.Cleanup(func() { isTerminalFn = oldIsTTY })
	isTerminalFn = func(_ *os.File) bool { return false }
...
func TestFromJKS_CLI_PasswordFile(t *testing.T) {
	oldIsTTY := isTerminalFn
	t.Cleanup(func() { isTerminalFn = oldIsTTY })
	isTerminalFn = func(_ *os.File) bool { return false }
	...
	_ = os.WriteFile(pwFile, []byte("changeit\n"), 0o600)
	...
	cmd.SetErr(&bytes.Buffer{})
	outDir := filepath.Join(dir, "extracted")
	cmd.SetArgs([]string{"from-jks", jksPath, outDir, "--password-file", pwFile, "--json"})
